...
create or replace procedure dm.fill_account_turnover_f ( --to_date убрала
   i_OnDate date
)
language plpgsql    
as $$
...
-- анонимный блок для вызова процедуры в цикле
do $$
declare
	date_jan date;
begin
	for date_jan in select generate_series('2018-01-01'::date, '2018-01-31'::date, '1 day'::interval) loop
		call fill_account_turnover_f(date_jan);
	end loop;
end;
$$

-- проверка данных в витрине оборотов
select * from dm_account_turnover_f datf ;

--delete from dm_account_turnover_f ;

-- проверка процесса логирования
select * from lg_messages lm ;

--delete from lg_messages ;

-- процедура для витрины 101-й формы
          ...
          sum(  case 
           ...
           case
              when acc_d.char_type = 'A' and cur.currency_code in ('643', '810') 
                then sum( case when cur.currency_code in ('643', '810')
                               then b.balance_out
                               else 0 end )
                     - sum( case when cur.currency_code in ('643', '810')
                                 then at.credit_amount_rub
                                 else 0 end )
                     + sum( case when cur.currency_code in ('643', '810')
                                 then at.debet_amount_rub
           ...
           case
              when acc_d.char_type = 'A' and cur.currency_code not in ('643', '810') 
                then sum( case when cur.currency_code not in ('643', '810')
                               then b.balance_out * exch_r.reduced_cource
                               else 0 end )
                     - sum( case when cur.currency_code not in ('643', '810')
                                 then at.credit_amount_rub
                                 else 0 end )
                     + sum( case when cur.currency_code not in ('643', '810')
                                 then at.debet_amount_rub
...
call fill_f101_round_f('2018-01-31');
-- проверка витрины 101-й формы
select * from dm_f101_round_f dfrf ;
--delete from dm_f101_round_f ;
--select (date_trunc('month', '2018-01-31'::date) - INTERVAL '1 day');
select * from lg_messages lm ;
--delete from lg_messages;
...
create or replace procedure dm.fill_f101_round_f_old ( 
  i_OnDate  date
)
language plpgsql    
as $$
...
begin
  ...
  create table dm_f101_round_f_old(
	from_date date,
	to_date date,          
    chapter char(1),           
    ledger_account char(5),
...
call fill_f101_round_f_old('2018-01-31');
select * from dm_f101_round_f_old;
select from_date,  to_date,        
    ...
    r_balance_out_rub ,
    r_balance_out_val,
    r_balance_out_total from dm_f101_round_f_old
except
select 
 from_date,  to_date,        
    chapter,           
    ...
    r_balance_out_val,
    r_balance_out_total
from dm_f101_round_f dfrf ;

--delete from dm_f101_round_f_old;
